   ...
   stage('Check code coverage') {
            steps {
                script {
                    ...
                    def componentKey = "com.sesame:order"
      ...
      steps {
          ...
          sh 'docker build -t jiheneayoub1/order-service:${VERSION} .'
          sh 'docker push jiheneayoub1/order-service:${VERSION}'
      }
    } 
    ...
    stage('Update Image Tag in GitOps') {
      steps {
        ...
        script {
       sh '''
          sed -i "s/image:.*/image: jiheneayoub1\\/order-service:${VERSION}/" aws/order-manifest.yml
        '''
